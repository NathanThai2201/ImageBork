...
def LineHalftone(image, palette):

    matrix = [[-4,-4,0,4,8,12,16,20,20,16,12,8,4,0]]
    length = len(matrix[0])
    h, w, _ = image.shape
    for row in range(h-1):
        for col in range(1, w-1, 1):
            oldpixel = image[row, col].copy()
            spread = 0.8
            pixel = oldpixel + spread*((1/16)*matrix[row%1][col%length]-0.5)
            newpixel = nearest(palette, pixel)
            image[row, col] = newpixel
    # Ensure pixel values are clipped to the valid range [0, 1]
    image = np.clip(image, 0, 1)
...
def main2(image):
    ...
    # Call dithering function
    img = LineHalftone(image, palette)
