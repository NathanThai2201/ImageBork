...
global_palette = [[0.98,0.576,0.561],
                     [1, 0.737, 0.259],
                     [0.949, 0.6, 0.886],
                     [0.361,0.922,0.961],
                     [0.537,0.792,1]]
                ...
                if thresholds[1] < mean < thresholds[2]:
                    for a in np.arange(0,4,1):
                        for b in np.arange(0,6,1):
                            ...
                            img_out_pad[i+a,j+b,1]=1
                            img_out_pad[i+a,j+b,2]=1
        ...
        for i in np.arange(pad,h-pad,8):
            for j in np.arange(pad,w-pad,12):
                ...
                if thresholds[1] < mean < thresholds[2]:
                    ...
                    if np.all(img_out_pad[i,j] == [1,1,1]) \
                    and np.all(img_out_pad[i+4,j+0] == [1,1,1]) \
                    and np.all(img_out_pad[i+0,j+6] == [1,1,1]) \
                    and np.all(img_out_pad[i+4,j+6] == [1,1,1]):
        ...
        for i in np.arange(pad,h-pad,16):
            for j in np.arange(pad,w-pad,24):
                ...
                if thresholds[1] < mean < thresholds[2]:
                    # method: check top left of each cell to detect white
                    if np.all(img_out_pad[i,j] == [1,1,1]) \
                    and np.all(img_out_pad[i+8,j+0] == [1,1,1]) \
                    and np.all(img_out_pad[i+0,j+12] == [1,1,1]) \
                    and np.all(img_out_pad[i+8,j+12] == [1,1,1]) \
                    and np.all(img_out_pad[i+8,j+23] == [1,1,1]) \
                    and np.all(img_out_pad[i+0,j+23] == [1,1,1]) \
                    and np.all(img_out_pad[i+15,j+0] == [1,1,1]) \
                    and np.all(img_out_pad[i+15,j+12] == [1,1,1]):
        ...
        for i in np.arange(pad,h-pad,32):
            for j in np.arange(pad,w-pad,48):
                ...
                if thresholds[1] < mean < thresholds[2]:
                    # method: check top left of each cell to detect white
                    if np.all(img_out_pad[i,j] == [1,1,1]) \
                    and np.all(img_out_pad[i+16,j+0] == [1,1,1]) \
                    and np.all(img_out_pad[i+0,j+24] == [1,1,1]) \
                    and np.all(img_out_pad[i+16,j+24] == [1,1,1]) \
                    and np.all(img_out_pad[i+16,j+47] == [1,1,1]) \
                    and np.all(img_out_pad[i+0,j+47] == [1,1,1]) \
                    and np.all(img_out_pad[i+31,j+0] == [1,1,1]) \
                    and np.all(img_out_pad[i+31,j+24] == [1,1,1]):
                        
                        for a in np.arange(0,32,1): # pure white
